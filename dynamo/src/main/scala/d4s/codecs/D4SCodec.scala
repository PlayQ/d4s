...
object D4SCodec {
  def apply[A](implicit derivedCodec: DerivationDerivedCodec[A]): D4SCodec[A] = new D4SCodec[A] {
    ...
    override def decodeAttribute(item: AttributeValue): Either[CodecsUtils.DynamoDecoderException, A]     = derivedCodec.dec.decodeAttribute(item)
  }
