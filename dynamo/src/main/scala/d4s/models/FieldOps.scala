...
object FieldOps {
  ...
  final class TypedFieldOps[T](private val name: String) extends AnyVal {
    ...
    def isIn(set: Set[T])(implicit enc: DynamoAttributeEncoder[T]): Condition.in[T] = {
      Condition.in(name, set)
    }

    def between(left: T, right: T)(implicit enc: DynamoAttributeEncoder[T]): Condition.between[T] = {
      Condition.between(name, left, right)
    }

    def >(threshold: T)(implicit enc: DynamoAttributeEncoder[T]): Condition.logical[T] = {
