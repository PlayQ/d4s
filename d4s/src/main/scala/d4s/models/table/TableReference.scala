...
object TableReference {
  ...
  implicit final class TableReferenceOps(private val table: TableReference) extends AnyVal {
    ...
    def scan(index: TableIndex[Nothing, Nothing]): DynamoQuery[Scan, ScanResponse] = Scan(table).withIndex(index).toQuery
    ...
    def query(index: TableIndex[Nothing, Nothing]): DynamoQuery[Query, QueryResponse] = Query(table).withIndex(index).toQuery
    ...
    def query(index: TableIndex[Nothing, Nothing], key: Map[String, AttributeValue]): DynamoQuery[Query, QueryResponse] =
      Query(table).withIndex(index).withKey(key).toQuery
    def query[H](index: TableIndex[H, Nothing], hashKey: H): DynamoQuery[Query, QueryResponse] =
      Query(table).withIndex(index).withKeyField(index.key.hashKey)(hashKey).toQuery
    ...
    def queryDeleteBatch(index: TableIndex[Nothing, Nothing]): DynamoQuery[QueryDeleteBatch, List[BatchWriteItemResponse]] =
    ...
    def queryDeleteBatch(index: TableIndex[Nothing, Nothing], key: Map[String, AttributeValue]): DynamoQuery[QueryDeleteBatch, List[BatchWriteItemResponse]] =
      QueryDeleteBatch(table).withIndex(index).withKey(key).toQuery
    def queryDeleteBatch[H](index: TableIndex[H, Nothing], hashKey: H): DynamoQuery[QueryDeleteBatch, List[BatchWriteItemResponse]] =
