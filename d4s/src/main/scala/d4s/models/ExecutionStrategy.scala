...
trait ExecutionStrategy[DR <: DynamoRequest, -Dec, +A] extends ExecutionStrategy.Dependent[DR, Dec, FThrowable[_[+ _, + _], A]] {
...
object ExecutionStrategy {
  final case class StrategyInput[F[+_, +_], DR <: DynamoRequest, +Dec](
    ...
    streamExecutionWrapper: F[Throwable, _] ~> F[Throwable, _],
  ...
  object StrategyInput {
    def apply[F[+_, +_]: Async2: Temporal2, DR <: DynamoRequest, Dec](
      ...
      streamExecutionWrapper: F[Throwable, _] ~> F[Throwable, _]                 = FunctionK.id[F[Throwable, _]],
  ...
  private[models] type StreamFThrowable[+F[+_, +_], +A] = Stream[F[Throwable, _], A]
  ...
  trait Streamed[DR <: DynamoRequest, -Dec, +A] extends ExecutionStrategy.Dependent[DR, Dec, StreamFThrowable[_[+ _, + _], A]] {
