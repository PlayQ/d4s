...
object DynamoException {
  ...
  object DynamoQueryException {
    def apply(cause: Throwable): DynamoQueryException = cause match {
      case DynamoException(message, cause) => DynamoQueryException(s"Dynamo query failed due to error=$message", cause)
      case cause                           => DynamoQueryException(s"Dynamo query failed due to error=${cause.getMessage}", cause)
    }
    def apply(queryName: String, cause: Throwable): DynamoQueryException = cause match {
      case DynamoException(message, cause) => new DynamoQueryException(s"Dynamo query `$queryName` failed due to error=$message", cause)
      case cause                           => new DynamoQueryException(s"Dynamo query `$queryName` failed due to error=${cause.getMessage}", cause)
    }
  }
}
